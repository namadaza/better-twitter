...
if (!WIKIART_ACCESS || !WIKIART_SECRET) {
  console.error('WIKIART_ACCESS_KEY and WIKIART_SECRET_KEY are required for this script');
  process.exit(1);
}
...
async function run() {
  ...
  const manifest = { generated_at: new Date().toISOString(), source: 'wikiart', count: finalItems.length, items: finalItems };
